...
class User(Base):
    ...
    googlevoice = Column(Boolean, default=False) # App
...
class AttackData(Base):
    ...
    id = Column(Integer, primary_key=True)
    planet_id = Column(Integer, ForeignKey(Planet.id, ondelete='cascade'), index=True)
    attack_id = Column(Integer, ForeignKey(Attack.id, ondelete='cascade'), index=True)
    unique = UniqueConstraint('planet_id','attack_id')
Attack.planets = dynamic_loader(AttackData, backref="attack")
